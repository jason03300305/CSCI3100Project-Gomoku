...
def display_timer():
    ...
    if elapsed_time >= 3600:
        ...
        timer_text = f"Playtime: Not Started Yet"
...
def display_timer15s():
    ...
    if is_green:
        timer_text15 = f"Player1's Round Timer: Not Started Yet"
    else:
        timer_text15 = f"Player2's Round Timer: Not Started Yet"
...
while running:
    ...
    # Draw Room ID
    RoomID_text = default_font2.render("Room Number: 4973", True, "white")
    RoomID_rect = RoomID_text.get_rect(center=(1050, 20))  # Get the rectangle for the menu text and center it    
    screen.blit(RoomID_text, RoomID_rect)  # Draw the  text on the screen
    ...
    Player1_rect = Player1_text.get_rect(center=(1050, 90))  # Get the rectangle for the menu text and center it    
    screen.blit(Player1_text, Player1_rect)  # Draw the  text on the screen

    # Draw Player2 information, such as chess color, name
    pos_white = (930,160)
    if not is_green:
        Player2_text = default_font2.render("Player2: Waiting...", True, "green")
        pygame.draw.circle(screen, "green", pos_white, 21)
    else:
        pygame.draw.circle(screen, "white", pos_white, 20)
        Player2_text = default_font2.render("Player2: Waiting...", True, "white")

    Player2_rect = Player2_text.get_rect(center=(1070, 160))  # Get the rectangle for the menu text and center it    
    screen.blit(Player2_text, Player2_rect)  # Draw the  text on the screen


    # Draw the chat box on the screen       
    Chat_text = default_font2.render("Chat Box", True, "white")
